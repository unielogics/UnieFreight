...
export default function InboxPage() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (!body) return
    const to = selectedThreadKey ? replyToEmail : composeTo.trim()
    const subject = selectedThreadKey ? replySubject : composeSubject.trim()
    ...
    try {
      await sendMailboxEmail({
        ...
        subject,
        body,
        ...(selectedThreadKey && selectedThreadKey.startsWith('freight:') ? { threadId: selectedThreadKey, inReplyTo: selectedThreadKey } : {}),
      })
      setComposeBody('')
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-gray-600 text-sm mt-1">
          Messages with warehouses. Reply to threads or start a new message.
        </p>
      </div>
          ...
          <div className="w-80 border-r border-gray-200 flex flex-col max-h-96 overflow-y-auto">
            ...
            {threads.length === 0 ? (
              ...
              threads.map((t) => (
                ...
                >
                  <div className="text-sm truncate text-gray-900">{t.latest?.subject || '(No subject)'}</div>
                  <div className="text-xs text-gray-500 truncate">
                    {t.latest?.direction === 'inbound' ? (t.latest.fromEmail || '—') : (t.latest?.toEmails?.[0] || '—')} · {t.messages.length} message{t.messages.length !== 1 ? 's' : ''}
                  </div>
                </button>
              ))
            )}
          ...
          <div className="flex-1 flex flex-col min-h-0">
            {selectedThreadKey ? (
              ...
              <div className="flex-1 flex items-center justify-center text-gray-500 text-sm">
                Select a thread or compose a new message below.
              </div>
            )}

            <form onSubmit={handleSend} className="p-4 border-t border-gray-200 bg-gray-50">
              <h3 className="text-sm font-semibold text-gray-900 mb-2">{selectedThreadKey ? 'Reply' : 'New message'}</h3>
              {!selectedThreadKey && (
                <>
                  <input
                    type="email"
                    placeholder="To (warehouse email)"
                    value={composeTo}
              ...
              >
                ...
                {sending ? 'Sending…' : selectedThreadKey ? 'Reply' : 'Send'}
              </button>
            </form>
          </div>
        </div>
